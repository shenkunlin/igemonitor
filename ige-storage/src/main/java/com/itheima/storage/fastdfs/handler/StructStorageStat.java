...
public class StructStorageStat extends StructBase {
   ...
   */
  public void setFields(byte[] bs, int offset,ClientGlobal clientGlobal) {
    this.status = byteValue(bs, offset, fieldsArray[FIELD_INDEX_STATUS]);
    this.id = stringValue(bs, offset, fieldsArray[FIELD_INDEX_ID],clientGlobal);
    this.ipAddr = stringValue(bs, offset, fieldsArray[FIELD_INDEX_IP_ADDR],clientGlobal);
    this.srcIpAddr = stringValue(bs, offset, fieldsArray[FIELD_INDEX_SRC_IP_ADDR],clientGlobal);
    this.domainName = stringValue(bs, offset, fieldsArray[FIELD_INDEX_DOMAIN_NAME],clientGlobal);
    this.version = stringValue(bs, offset, fieldsArray[FIELD_INDEX_VERSION],clientGlobal);
