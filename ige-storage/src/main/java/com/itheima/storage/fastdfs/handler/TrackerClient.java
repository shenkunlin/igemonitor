...
public class TrackerClient {
    ...
    private ClientGlobal global;

    /**
     * constructor with global tracker group
     */
    public TrackerClient(ClientGlobal global) {
        this.tracker_group = global.g_tracker_group;
        this.global=global;
    }
     ...
     */
    public StorageServer getStoreStorage(TrackerServer trackerServer,ClientGlobal clientGlobal) throws IOException, MyException {
        final String groupName = null;
        return this.getStoreStorage(trackerServer, groupName,clientGlobal);
    }
     ...
     */
    public StorageServer getStoreStorage(TrackerServer trackerServer, String groupName,ClientGlobal clientGlobal) throws IOException, MyException {
        byte[] header;
        String ip_addr;
        int port;
        byte cmd;
        ...
        connection = trackerServer.getConnection(clientGlobal);
        ...
        try {
            ...
            if (groupName != null && groupName.length() > 0) {
                ...
                bs = groupName.getBytes(global.g_charset);
     ...
     */
    public StorageServer[] getStoreStorages(TrackerServer trackerServer, String groupName,ClientGlobal clientGlobal) throws IOException, MyException {
        byte[] header;
        String ip_addr;
        int port;
        byte cmd;
    ...
    public StorageServer getFetchStorage(TrackerServer trackerServer,
                                         String groupName, String filename,ClientGlobal clientGlobal) throws IOException, MyException {
        ServerInfo[] servers = this.getStorages(trackerServer, ProtoCommon.TRACKER_PROTO_CMD_SERVICE_QUERY_FETCH_ONE,
                groupName, filename,clientGlobal);
        if (servers == null) {
            return null;
        } else {
    ...
    protected ServerInfo[] getStorages(TrackerServer trackerServer,
                                       byte cmd, String groupName, String filename,ClientGlobal clientGlobal) throws IOException, MyException {
        byte[] header;
        byte[] bFileName;
        byte[] bGroupName;
        byte[] bs;
        ...
        try {
            bs = groupName.getBytes(global.g_charset);
            bGroupName = new byte[ProtoCommon.FDFS_GROUP_NAME_MAX_LEN];
            bFileName = filename.getBytes(global.g_charset);
     ...
     * @return storage server Socket object, return null if fail
     */
    public StorageServer getFetchStorage1(TrackerServer trackerServer, String file_id,ClientGlobal clientGlobal) throws IOException, MyException {
        ...
        return this.getFetchStorage(trackerServer, parts[0], parts[1],clientGlobal);
    }
     ...
     * @return storage servers, return null if fail
     */
    public ServerInfo[] getFetchStorages1(TrackerServer trackerServer, String file_id,ClientGlobal clientGlobal) throws IOException, MyException {
        String[] parts = new String[2];
        this.errno = StorageClient1.split_file_id(file_id, parts);
        if (this.errno != 0) {
            return null;
        ...
        return this.getFetchStorages(trackerServer, parts[0], parts[1],clientGlobal);
    }
     ...
     */
    public StructGroupStat[] listGroups(TrackerServer trackerServer,ClientGlobal clientGlobal) throws IOException, MyException {
        byte[] header;
        String ip_addr;
        int port;
        byte cmd;
        ...
        try {
            ...
            return decoder.decode(pkgInfo.body, StructGroupStat.class, StructGroupStat.getFieldsTotalSize(),clientGlobal);
        } catch (IOException ex) {
            try {
                connection.close(clientGlobal);
            } catch (IOException ex1) {
     ...
     */
    public StructStorageStat[] listStorages(TrackerServer trackerServer, String groupName,ClientGlobal clientGlobal) throws IOException, MyException {
        final String storageIpAddr = null;
        return this.listStorages(trackerServer, groupName, storageIpAddr,clientGlobal);
    }
    ...
    public StructStorageStat[] listStorages(TrackerServer trackerServer,
                                            String groupName, String storageIpAddr,ClientGlobal clientGlobal) throws IOException, MyException {
        byte[] header;
        byte[] bGroupName;
        byte[] bs;
        int len;
        ...
        try {
            ...
            if (storageIpAddr != null && storageIpAddr.length() > 0) {
                bIpAddr = storageIpAddr.getBytes(global.g_charset);
            ...
            return decoder.decode(pkgInfo.body, StructStorageStat.class, StructStorageStat.getFieldsTotalSize(),clientGlobal);
        } catch (IOException ex) {
            try {
                connection.close(clientGlobal);
            } catch (IOException ex1) {
    ...
    private boolean deleteStorage(TrackerServer trackerServer,
        ...
        try {
            ...
            int ipAddrLen;
            byte[] bIpAddr = storageIpAddr.getBytes(global.g_charset);
            if (bIpAddr.length < ProtoCommon.FDFS_IPADDR_SIZE) {
                ipAddrLen = bIpAddr.length;
            } else {
                ipAddrLen = ProtoCommon.FDFS_IPADDR_SIZE - 1;
     ...
     * @return true for success, false for fail
     */
    public boolean deleteStorage(String groupName, String storageIpAddr,ClientGlobal clientGlobal) throws IOException, MyException {
        return this.deleteStorage(global.g_tracker_group, groupName, storageIpAddr,clientGlobal);
    }
    ...
    public boolean deleteStorage(TrackerGroup trackerGroup,
        ...
        for (serverIndex = 0; serverIndex < trackerGroup.tracker_servers.length; serverIndex++) {
            ...
            StructStorageStat[] storageStats = listStorages(trackerServer, groupName, storageIpAddr,clientGlobal);
        ...
        for (serverIndex = 0; serverIndex < trackerGroup.tracker_servers.length; serverIndex++) {
            ...
            if (!this.deleteStorage(trackerServer, groupName, storageIpAddr,clientGlobal)) {
                if (this.errno != 0) {
                    if (this.errno == ProtoCommon.ERR_NO_ENOENT) {
                        notFoundCount++;
